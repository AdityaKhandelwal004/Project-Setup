...
import {
  APICALL, ApiCall, FETCH_BASE_DATA,
  LOGIN, LOGOUT,
  PAGINATED_APICALL, PagedApiCall, action, fetchBaseData, logout,
  removeToken, updateToken,
} from '../actions';
import { HttpStatus } from '../../utils';
import { apiCall, login, ping } from '../../api';
import { MetaData } from '../../models';
import {
  Role, defaultAuthState, getStateFromToken, getToken,
} from '../reducers/auth';
...
export function* handleResponse(
  ...
  try {
    if (response.status === HttpStatus.Ok) {
      ...
      const authToken = response?.headers?.get('Authorization')?.split('Bearer ')[1];
      if (authToken) {
...
function* doApiCall<RequestProps, SuccessProps extends { _requestDate: Date },
  ...
  const { request: { requestProps: { endpoint } } } = event.payload;
  const date = new Date();
...
export function getPaginationParameters(filter: MetaData<any>): string {
  ...
  Object.keys(filters).forEach((key) => {
    filterParams = filterParams.concat(filters[key] ? [`filter[${key}]=${filters[key]}`] : []);
  });
    ...
    yield* handleResponse(
      ...
      function* (body) {
        ...
        if (newState?.role === Role.INVALID) {
          yield event?.payload?.reject({ message: 'error.login.invalidCredentials' });
        } else {
