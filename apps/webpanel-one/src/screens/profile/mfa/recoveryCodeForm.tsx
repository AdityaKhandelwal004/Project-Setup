...
} from '@wellCheck/components';
import { Checkbox } from '@mui/material';
import { Id } from '@wellCheck/models';
...
const RecoveryCodeForm: React.FC<Props> = ({
  ...
  const codes = recoveryCodeConfig?.data?.records?.map((item: { id: Id; code: string }) => item?.code)?.join('\n');
    ...
    <Form>
      ...
      <FormRow>
        <FormRowItem alignItems="center" justifyContent="center" gap="32px">
          {recoveryCodeConfig?.data?.records?.map((item: { id: Id; code: string }) => (
