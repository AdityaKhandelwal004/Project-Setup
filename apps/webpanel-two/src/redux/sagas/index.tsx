...
import { HttpStatus } from '@mono/utils';
...
import {
  ...
  ACTIVE_TWO_FACTOR_AUTHENTICATION,
  POST_LOGIN,
  hideLoader,
} from '../actions/index.tsx';
import type { ApiCall, PagedApiCall } from '../actions/index.tsx';
import { apiCall, login, signup, forgotPassword, ping, GET_PROFILE, twoFactorAuthentication } from '../../api.ts';
import {
  Role,
  defaultAuthState,
  getStateFromToken,
  getToken,
} from '../reducers/auth.tsx';
...
export function* doFetchBaseData(): Generator<any> {  
  ...
  yield* handleResponse(
    result,
    function* (response) {
      yield put(profileModuleActions.update(response || {}));
    },
    function* () {
    ...
    yield* handleResponse(
      ...
      function* (body: any, response: Response) {
        ...
        if (response.status === 400) {
          errorMessage = body?.message || 'Invalid signup data. Please check your information.';
          errors = body?.errors || {};
        } else if (response.status === 409) {
          errorMessage = body?.message || 'Email already exists. Please use a different email.';
        } else if (response.status === 422) {
          errorMessage = body?.message || 'Validation failed. Please check your information.';
          errors = body?.errors || {};
  ...
  try {
    ...
    yield* handleResponse(
      ...
      function* (body: any, response: Response) {
        ...
        if (response.status === 400) {
          errorMessage = body?.message || 'Invalid email address. Please check and try again.';
        } else if (response.status === 404) {
          errorMessage = body?.message || 'Email address not found. Please check and try again.';
        } else if (response.status === 429) {
          errorMessage = body?.message || 'Too many requests. Please wait before trying again.';
        } else if (response.status >= 500) {
          errorMessage = 'Server error. Please try again later.';
        }
...
export function* doTwofactorAuthentication(event: any): Generator<any> {
  try {
    const result: any = yield call(twoFactorAuthentication, event?.payload?.formData);
    yield* handleResponse(
      result,
      function* (body) {
        const newState = getStateFromToken(defaultAuthState, body?.token);
...
export function* rootSaga(): SagaIterator<void> {
  ...
  yield takeEvery(ACTIVE_TWO_FACTOR_AUTHENTICATION, doTwofactorAuthentication);
}
